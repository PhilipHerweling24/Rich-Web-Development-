...
//Observable for all buttons in my web app
fromEvent(document.getElementById("notes"), 'click').subscribe((e) => {

  //check if delete, edit or save buttons are clicked and 
  //calls there function and passes through there required info
  if (e.target.className == "note_delete") {
      let delete_button = e.target;
      note_delete(delete_button);
  }

  if (e.target.className == "note_edit") {
      let edit_button = e.target;
      note_edit(edit_button);
  }

  if (e.target.className == "note_save") {
      let save_button = e.target;
      note_save(save_button);
  }

});

//const deleteButton = fromEvent(deleteBTN, 'click');
...
submit.subscribe( () => 
 {
    ...
    divTag.innerHTML =  '<div class = "note-content">' +
                            ...
                            '<button class="note_delete">Delete Note</button>' +
                            '<button class="note_edit">Edit Note</button>' +
                            '</div>';
  ...
  function note_edit(edit_button) {
    ...
    content.innerHTML = '<label for="title">Title' +
                            ...
                            '</label>' +
                            '<button class="note_save">Save Changes</button>';
